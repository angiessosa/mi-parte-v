...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 21-10-2024 a las 06:18:10
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarTaller` (IN `p_centroFormacion` VARCHAR(100), IN `p_jornada` VARCHAR(50), IN `p_coordinacion` VARCHAR(100), IN `p_numFicha` VARCHAR(11), IN `p_tema` VARCHAR(50), IN `p_fechaYHora` DATETIME, IN `p_observaciones` TEXT, IN `p_idUsuario` INT)   BEGIN
    -- Insertar en la tabla taller
    INSERT INTO taller (centroFormacion, jornada, coordinacion, numFicha, tema, fechaYHora, observaciones)
    VALUES (p_centroFormacion, p_jornada, p_coordinacion, p_numFicha, p_tema, p_fechaYHora, p_observaciones);

    -- Obtener el último idTaller insertado
    SET @idTaller := LAST_INSERT_ID();

    -- Insertar el profesional en la tabla usuario_taller
    INSERT INTO usuario_taller (idUsuario, idTaller)
    VALUES (p_idUsuario, @idTaller);

    -- Insertar el horario en la tabla horarios
    INSERT INTO horarios (idUsuario, idTaller, fecha, horaInicio, horaFin)
    VALUES (p_idUsuario, @idTaller, DATE(p_fechaYHora), TIME(p_fechaYHora), ADDTIME(TIME(p_fechaYHora), '00:30:00'));  -- Añadiendo 30 minutos
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_coordinaciones` (IN `jornadaSeleccionada` VARCHAR(10))   BEGIN
    SELECT DISTINCT nombreCoordinacion 
    FROM ficha
    WHERE jornada = jornadaSeleccionada;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_fichas` (IN `jornadaSeleccionada` VARCHAR(10), IN `nombreCoordinacionSeleccionada` VARCHAR(100))   BEGIN
    SELECT numFicha 
    FROM ficha
    WHERE jornada = jornadaSeleccionada
    AND nombreCoordinacion = nombreCoordinacionSeleccionada;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_jornadas` ()   BEGIN
    SELECT DISTINCT jornada FROM ficha;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_profesionales_disponibles` (IN `fechaTaller` DATETIME, IN `idFichaSeleccionada` INT)   BEGIN
    SELECT u.idUsuario, u.nombres, u.apellidos 
    FROM usuarios u
    JOIN usuario_taller ut ON u.idUsuario = ut.idUsuario
    WHERE u.idRol = (SELECT idRol FROM rol WHERE tipoRol = 'profesional')
    AND u.idUsuario NOT IN (
        -- Excluir los profesionales con talleres ya agendados en ese horario
        SELECT idUsuario FROM taller t
        JOIN usuario_taller ut ON t.idTaller = ut.idTaller
        WHERE t.fechaYHora BETWEEN fechaTaller AND DATE_ADD(fechaTaller, INTERVAL 30 MINUTE)
    )
    AND EXISTS (
        -- Verificar que el profesional esté disponible para la ficha seleccionada
        SELECT 1 FROM horarios h
        WHERE h.idUsuario = u.idUsuario
        AND h.fichas = idFichaSeleccionada
    );
END$$
...
(12, 'Centro Principal', 'mañana', 'Informática', '2898754', 'autocuidado', '2024-10-21 08:00:00', 'Taller introductorio');
...
(42, 'C.C', '1111', 'Angie', 'pruebsa2', 'angiesod@gmail', '$2b$12$v5iWDE69yOL.VYVkyImopeY2GxNwRollseqh4O/Hm4JKGn5wxuZJW', 1, 'pendiente'),
(43, 'C.C', 'rwrw', 'rwrwr', 'rwrwr', 'rwrw@j', '$2b$12$vkxuj/szJ13wJfxURrEVlueSGZILTGwuYC2tmy5FCcQlhSWfPMLp2', 1, 'pendiente');
