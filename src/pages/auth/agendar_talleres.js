    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const taller = {
                centroFormacion,
                jornada,
                coordinacion: selectedCoordinacion,  // Mantén esto como string si es necesario
                numFicha: selectedNumFicha,  // Si es un string, manténlo así
                tema: selectedTematica,  // Asegúrate de que esto sea el nombre del tema, no el ID
                fechaYHora: fechaYHora.toISOString(),
                observaciones
            };
    
            console.log("Datos de taller enviados:", taller);
    
            const tallerResponse = await axios.post('http://localhost:8000/Creartaller', taller);
    
            const idTaller = tallerResponse.data.idTaller;
    
            if (idTaller && selectedProfesional) {
                await axios.post('http://localhost:8000/usuario_taller', {
                    idUsuario: parseInt(selectedProfesional, 10),
                    idTaller: idTaller
                });
            }
